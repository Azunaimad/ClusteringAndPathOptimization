...
public class AntColonyOptimization {
    ...
    protected void countNewPheromones(){
        ...
        for (int i=0; i<numberOfAnts; i++) {
            for (int j = 0; j < numberOfTowns; j++) {
                simpleAntPher[ants[i].getAntRoute()[j]][ants[i].getAntRoute()[j+1]] +=
        ...
        for(int i=0; i<numberOfTowns; i++)
            eliteAntPher[ants[bestAntRouteIndex].getAntRoute()[i]]
                    [ants[bestAntRouteIndex].getAntRoute()[i+1]] = qParameter / bestLength;
    ...
    public int[] getBestRoute(){
        return ants[bestAntRouteIndex].getAntRoute();
    }
